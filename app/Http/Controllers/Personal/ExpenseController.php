...
class ExpenseController extends Controller
{
    ...
    public function expenses()
    {
        ...
        $user = $this->getUser();
        // Get expenses
        $expenses = Expense::where('user_id', $user->id)->where('is_user', true)
            ->with('user', 'status', 'expenseAccount')->get();
        // return $expenses;
    ...
    public function expenseCreate()
    {
        ...
        // account types
        $billExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '163fa506-9762-422a-a981-cce20b21f1ad')->where('is_user', true)->with('accountType')
            ->get();
        $cashExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '8c0c1829-b6cf-4d38-b640-755db25460ae')->where('is_user', true)->with('accountType')
            ->get();
        $feesExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '17401c1e-1423-40bc-846a-008b0e72373c')->where('is_user', true)->with('accountType')
            ->get();
        $foodExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '008315c2-ee90-4e55-80cc-de2a8bc0472a')->where('is_user', true)->with('accountType')
            ->get();
        $healthExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', 'af7b5592-8c36-4746-b369-a3985c90fd0b')->where('is_user', true)->with('accountType')
            ->get();
        $homeLivingExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '1c523f60-ab8f-4dd7-88ca-a70863507a3b')->where('is_user', true)->with('accountType')
            ->get();
        $incomeExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '6943fd67-ba09-4fc3-986a-3550ae959b33')->where('is_user', true)->with('accountType')
            ->get();
        $kidsExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '84ccf3c6-74fb-4af9-b4b2-7bef9d0469b8')->where('is_user', true)->with('accountType')
            ->get();
        $leisureExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '55faadc5-6275-4d19-809e-dc56e555929f')->where('is_user', true)->with('accountType')
            ->get();
        $loansExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '6269dc50-cfc9-4b3f-8c91-a1adc6bb998e')->where('is_user', true)->with('accountType')
            ->get();
        $noExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', 'c7c1a0a0-8775-45a7-a84b-92a8dac302d3')->where('is_user', true)->with('accountType')
            ->get();
        $shoppingExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '83569f71-59dc-46f0-a92f-fdac4ad922aa')->where('is_user', true)->with('accountType')
            ->get();
        $transportExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '7b05bf74-08e0-4692-becd-799b11d24dba')->where('is_user', true)->with('accountType')
            ->get();
        $wealthCreationExpenseAccounts = ExpenseAccount::where('user_id', $user->id)
            ->where('account_type_id', '46089cb5-ef46-4d9f-af5c-9676d7a55ed4')->where('is_user', true)->with('accountType')
            ->get();
        ...
        // get frequencies
        $frequencies = Frequency::where("status_id", "c670f7a2-b6d1-4669-8ab5-9c764a1e403e")
...
//        return $request;
        ...
        return redirect()->route('personal.expense.show', $expense->id)
            ->withSuccess('Expense '.$expense->reference.' successfully created!');
    }
    ...
    public function expenseShow($expense_id)
    {
        ...
        $user = $this->getUser();
        // get expense
        $expense = Expense::where('user_id', $user->id)->where('is_user', true)->where('id', $expense_id)
            ->with('transfer', 'status', 'expenseItems', 'transaction', 'expenseAccount', 'frequency', 'user',
                'account', 'campaign', 'contact', 'expenseAccount', 'inventoryAdjustment', 'liability', 'sale',
                'sale', 'warehouse')->withCount('expenseItems')->first();
        // get payments
        $payments = Transaction::where('user_id', $user->id)->where('is_user', true)->where('expense_id', $expense->id)
            ->where('status_id', '2fb4fa58-f73d-40e6-ab80-f0d904393bf2')->with('expense', 'account', 'status')->get();
        // get pending payments
        $pendingPayments = Transaction::where('user_id', $user->id)->where('is_user', true)
            ->where('expense_id', $expense->id)->where('status_id', 'a40b5983-3c6b-4563-ab7c-20deefc1992b')
            ->with('expense', 'account', 'status')->get();
        // Pending to dos
        $pendingToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'expense')
            ->where('status_id', 'f3df38e3-c854-4a06-be26-43dff410a3bc')->where('expense_id', $expense->id)->get();
        // In progress to dos
        $inProgressToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'expense')
            ->where('status_id', '2a2d7a53-0abd-4624-b7a1-a123bfe6e568')->where('expense_id', $expense->id)->get();
        // Completed to dos
        $completedToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'expense')
            ->where('status_id', 'facb3c47-1e2c-46e9-9709-ca479cc6e77f')->where('expense_id', $expense->id)->get();
        // Overdue to dos
        $overdueToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'expense')
            ->where('status_id', '99372fdc-9ca0-4bca-b483-3a6c95a73782')->where('expense_id', $expense->id)->get();
    ...
    public function expenseUpdate(Request $request, $expense_id)
    {
        ...
        $expenseItemIds = ExpenseItem::where('expense_id', $expense->id)->whereNotIn('id', $expenseProducts)
        ...
        return redirect()->route('personal.expense.show', $expense->id)
            ->withSuccess('Expense '.$expense->reference.' successfully updated!');
    }
    ...
    public function transactions()
    {
        ...
        $user = $this->getUser();
        // Get transactions
        $transactions = Transaction::where('user_id', $user->id)->where('is_user', true)->with('user', 'status',
            'account', 'expense')->get();
    ...
    public function transactionStore(Request $request)
    {
        ...
        return redirect()->route('personal.expense.show', $transaction->expense_id)
            ->withSuccess('Expense '.$transaction->reference.' successfully updated!');
    ...
    public function payments()
    {
        // User
        $user = $this->getUser();
        $payments = Payment::where('user_id', $user->id)->where('is_user', true)
            ->with('user', 'status', 'account')->get();
        ...
        if($request->is_sale == "on"){
            ...
            foreach($saleProducts as $saleProduct){
                ...
                $inventory = Inventory::where('product_id', $saleProduct->product_id)
                    ->where('warehouse_id', $saleProduct->warehouse_id)->first();
    ...
    public function paymentShow($payment_id)
    {
        ...
        $user = $this->getUser();
        // Get contact type
        $payment = Payment::with('user', 'status', 'refunds.account', 'loan', 'sale')
            ->where('id', $payment_id)->where('user_id', $user->id)->where('is_user', true)->first();
        // Pending to dos
        $pendingToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'payment')
            ->where('status_id', 'f3df38e3-c854-4a06-be26-43dff410a3bc')->where('payment_id', $payment->id)->get();
        // In progress to dos
        $inProgressToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'payment')
            ->where('status_id', '2a2d7a53-0abd-4624-b7a1-a123bfe6e568')->where('payment_id', $payment->id)->get();
        // Completed to dos
        $completedToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'payment')
            ->where('status_id', 'facb3c47-1e2c-46e9-9709-ca479cc6e77f')->where('payment_id', $payment->id)->get();
        // Overdue to dos
        $overdueToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'payment')
            ->where('status_id', '99372fdc-9ca0-4bca-b483-3a6c95a73782')->where('payment_id', $payment->id)->get();
    ...
    public function refunds()
    {
        ...
        $user = $this->getUser();
        // refunds
        $refunds = Refund::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'account')
            ->get();
    ...
    public function refundShow($refund_id)
    {
        ...
        $user = $this->getUser();
        // Get contact type
        $refund = Refund::with('user', 'status', 'account', 'payment')->where('id', $refund_id)
            ->where('user_id', $user->id)->where('is_user', true)->first();
        // Pending to dos
        $pendingToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'refund')
            ->where('status_id', 'f3df38e3-c854-4a06-be26-43dff410a3bc')->where('refund_id', $refund->id)->get();
        // In progress to dos
        $inProgressToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'refund')
            ->where('status_id', '2a2d7a53-0abd-4624-b7a1-a123bfe6e568')->where('refund_id', $refund->id)->get();
        // Completed to dos
        $completedToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'refund')
            ->where('status_id', 'facb3c47-1e2c-46e9-9709-ca479cc6e77f')->where('refund_id', $refund->id)->get();
        // Overdue to dos
        $overdueToDos = ToDo::where('user_id', $user->id)->where('is_user', true)->with('user', 'status', 'refund')
            ->where('status_id', '99372fdc-9ca0-4bca-b483-3a6c95a73782')->where('refund_id', $refund->id)->get();
