    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content animated bounceInRight">
            ...
            <div class="modal-body">
                <form method="post" action="{{ route('admin.user.store') }}" autocomplete="off" class="form-horizontal form-label-left">
                    @csrf
                            ...
                            <div class="has-warning">
                                <select name="role" data-placeholder="Choose a role..." class="chosen-select"  tabindex="2">
                                    ...
                                    @foreach($roles as $role)
                                        <option @isset($roleExists) @if($roleExists->id == $role->id) selected @endif @endisset value="{{encrypt($role->id)}}">{{$role->name}}</option>
                                    @endforeach()
                                </select>
                                <i>role</i>
                            </div>
